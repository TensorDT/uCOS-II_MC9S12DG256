...
*                                           TASK RETURN HOOK
*
* Description: This function is called when a task is returned.
*
* Arguments  : ptcb   is a pointer to the task control block of the task being returned.
*
* Note(s)    : 1) Interrupts are disabled during this call.
...
#ifdef ORIG_VERSION
_NEAR OS_STK *OSTaskStkInit (void (*task)(void *pd), void *p_arg, OS_STK *ptos, INT16U opt)
{
    INT8U  *bstk;
    ...
    *--bstk = (INT8U)(((INT32U)task));              /* Save the PPAGE register                                */

    return ((OS_STK *)bstk);                        /* Return pointer to new top-of-stack                     */
}

#else
_NEAR OS_STK *OSTaskStkInit (
    void (*task)(void *pd),       // task start address
    void *p_arg,                  // task argument pointer
    OS_STK *ptos,                 // task top of stack pointer
    INT16U opt)                   // options
{
    INT8U  *bstk;
    addr32_UN addrTemp_UN;
	value16_UN valueTemp_UN;
//    OS_CPU_SR cpu_sr;
    ...
    // Simulate call to function with an argument (i.e., p_arg)
    // For MC9S12DG256, the argument pointer would be passed via the "D" register
    // (not on the stack) so all we need to stack is a "return address".
    addrTemp_UN.addr = (INT32U)OSTaskAbort;
    *--bstk = addrTemp_UN.addr32_ST.byte0;       // address low byte  (simulated calling fn)
    *--bstk = addrTemp_UN.addr32_ST.byte1;       // address high byte (simulated calling fn)
    // Not sure if we need the page
    *--bstk = addrTemp_UN.addr32_ST.byte2;       // address page      (simulated calling fn)

    // Simulate ISR vector by stacking the registers that are automatically pushed
    // by the processor when it recognizes and starts servicing an interrupt
    addrTemp_UN.addr = (INT32U)task;
    *--bstk = addrTemp_UN.addr32_ST.byte0;       // address low byte  (task)
    *--bstk = addrTemp_UN.addr32_ST.byte1;       // address high byte (task)

    *--bstk = (INT8U)0x22;                       // Y Register low byte  (simulated)
    *--bstk = (INT8U)0x22;                       // Y Register high byte (simulated)

    *--bstk = (INT8U)0x11;                       // X Register low byte  (simulated)
    *--bstk = (INT8U)0x11;                       // X Register high byte (simulated)

    valueTemp_UN.value = (INT16U)p_arg;
    *--bstk = valueTemp_UN.value16_ST.byte1;     // A register / D register high task arg
    *--bstk = valueTemp_UN.value16_ST.byte0;     // B register / D register low  task arg

    *--bstk = 0xC0;                              // CCR Register (Disable STOP instruction and XIRQ)
                                                 //   (NOT the actual current CCR value)

    *--bstk = addrTemp_UN.addr32_ST.byte2;       // address page (task)
